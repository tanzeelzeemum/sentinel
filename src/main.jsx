...
import { Bell, ClipboardList, FileText, LayoutDashboard, LockKeyhole, Menu, Shield, Users, X } from 'lucide-react'
...
import { getAlerts, getCases, getProfile, getReports } from './api/cases'
...
function Dashboard() { const [cases, setCases] = useState([]); const [reports, setReports] = useState([]); const [alerts, setAlerts] = useState([]); const [loading, setLoading] = useState(true); const [error, setError] = useState(''); useEffect(() => { Promise.all([getCases(), getReports(), getAlerts()]).then(([casesPayload, reportsPayload, alertsPayload]) => { setCases(Array.isArray(casesPayload.cases) ? casesPayload.cases : Array.isArray(casesPayload.data) ? casesPayload.data : []); setReports(Array.isArray(reportsPayload.reports) ? reportsPayload.reports : []); setAlerts(Array.isArray(alertsPayload.alerts) ? alertsPayload.alerts : []); }).catch(err => setError(err.message)).finally(() => setLoading(false)) }, []); const priority = cases.filter(item => ['high', 'critical'].includes(item.threatLevel)); const recentCases = [...cases].sort((a,b) => new Date(b.createdAt) - new Date(a.createdAt)).slice(0, 5); const recentReports = [...reports].sort((a,b) => new Date(b.createdAt) - new Date(a.createdAt)).slice(0, 5); return <section className="content px-4 py-7 md:px-10 md:py-10"><p className="eyebrow">Operational overview</p><h1>Intelligence Dashboard</h1><p className="muted">Live case, report, and alert activity for your workspace.</p>{error && <p className="form-error rounded-md p-3">{error}</p>}{loading ? <p className="muted">Loading dashboard...</p> : <><div className="stats grid gap-4 md:grid-cols-2 xl:grid-cols-5"><Stat label="Total Cases" value={cases.length} /><Stat label="Active Cases" value={cases.filter(item => ['open', 'under-review'].includes(item.status)).length} /><Stat label="Critical Alerts" value={alerts.filter(alert => ['critical', 'Critical'].includes(alert.threatLevel) && alert.status !== 'Resolved').length} /><Stat label="Open Cases" value={cases.filter(item => item.status === 'open').length} /><Stat label="Total Reports" value={reports.length} /></div><section className="case-section mb-5"><h2>Operational Workflow</h2><div className="workflow-grid grid gap-3 md:grid-cols-4"><WorkflowStep title="Review Dashboard" text="Check current case, report, and alert activity." to="/dashboard" /><WorkflowStep title="Open or Create Case" text="Review authorized cases or start a new case." to="/cases" /><WorkflowStep title="Add Incident Report" text="Attach a fictional incident report to a case." to="/reports/new" /><WorkflowStep title="Review Threat and Alerts" text="Confirm high-priority alerts and assessment status." to="/alerts" /></div></section><section className="case-section"><h2>Priority Cases</h2>{priority.length === 0 ? <p className="muted">No priority cases.</p> : <CaseTable reports={priority} />}</section><section className="case-section dashboard-recent"><h2>Recent Cases</h2>{recentCases.length === 0 ? <p className="muted">No cases created yet.</p> : <CaseTable reports={recentCases} />}</section><section className="case-section dashboard-recent"><h2>Recent Reports</h2>{recentReports.length === 0 ? <p className="muted">No reports created yet.</p> : <ReportTable reports={recentReports} />}</section></>}</section> }
...
function ReportTable({ reports }) { return <div className="table-wrap overflow-x-auto"><table><thead><tr><th>REPORT</th><th>CASE</th><th>INCIDENT DATE</th><th>REPORTING OFFICER</th></tr></thead><tbody>{reports.map(report => <tr key={report._id}><td><b>{report.title}</b></td><td>{report.caseId?.caseNumber ? `${report.caseId.caseNumber} - ${report.caseId.title}` : 'Unassigned'}</td><td>{report.incidentDate ? new Date(report.incidentDate).toLocaleDateString() : '-'}</td><td>{report.submittedBy?.name || report.submittedBy?.email || report.submittedByEmail}</td></tr>)}</tbody></table></div> }
function WorkflowStep({ title, text, to }) { return <article className="rounded-md border border-slate-200 bg-white p-4"><h3 className="mb-2 text-sm font-bold text-slate-800">{title}</h3><p className="mb-3 text-sm text-slate-500">{text}</p><Link className="secondary-button inline-flex" to={to}>Open</Link></article> }
...
function App() { return <Routes><Route path="/" element={<Landing />} /><Route path="/login" element={<AuthPage />} /><Route path="/register" element={<AuthPage registerPage />} /><Route path="/forgot-password" element={<ForgotPassword />} /><Route path="/unauthorized" element={<Unauthorized />} /><Route element={<ProtectedRoute><DashboardLayout /></ProtectedRoute>}><Route path="/dashboard" element={<Dashboard />} /><Route path="/cases" element={<CasesPage />} /><Route path="/cases/new" element={<CaseFormPage />} /><Route path="/cases/:id" element={<CaseDetailsPage />} /><Route path="/cases/:id/edit" element={<CaseFormPage edit />} /><Route path="/reports" element={<ReportsPage />} /><Route path="/reports/new" element={<ReportFormPage />} /><Route path="/reports/:id" element={<ReportDetailsPage />} /><Route path="/reports/:id/edit" element={<ReportFormPage edit />} /><Route path="/alerts" element={<AlertsPage />} /><Route path="/profile" element={<ProfilePage />} /></Route><Route path="*" element={<NotFound />} /></Routes> }
