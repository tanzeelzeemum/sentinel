...
import { getAlerts, getCases } from './api/cases'
...
function DashboardLayout() {
  ...
  return <main className="app-shell"><Sidebar open={open} close={() => setOpen(false)} /><div className="workspace"><header className="topbar"><button className="menu-button" onClick={() => setOpen(true)} aria-label="Open menu"><Menu /></button><p>Sentinel</p><div className="top-user">{user?.email}</div></header><Outlet /></div></main>
}

function Dashboard() { const [reports, setReports] = useState([]); const [alerts, setAlerts] = useState([]); const [loading, setLoading] = useState(true); const [error, setError] = useState(''); useEffect(() => { Promise.all([getCases(), getAlerts()]).then(([casesPayload, alertsPayload]) => { setReports(Array.isArray(casesPayload.cases) ? casesPayload.cases : Array.isArray(casesPayload.data) ? casesPayload.data : []); setAlerts(Array.isArray(alertsPayload.alerts) ? alertsPayload.alerts : []); }).catch(err => setError(err.message)).finally(() => setLoading(false)) }, []); const priority = reports.filter(report => ['High', 'Critical'].includes(report.threatLevel)); const recent = [...reports].sort((a,b) => new Date(b.createdAt) - new Date(a.createdAt)).slice(0, 5); return <section className="content"><p className="eyebrow">Operational overview</p><h1>Intelligence Dashboard</h1><p className="muted">Live report and alert activity for your workspace.</p>{error && <p className="form-error">{error}</p>}{loading ? <p className="muted">Loading dashboard...</p> : <><div className="stats"><Stat label="Total Reports" value={reports.length} /><Stat label="Active Cases" value={reports.filter(report => report.status === 'Active').length} /><Stat label="Critical Alerts" value={alerts.filter(alert => alert.threatLevel === 'Critical' && alert.status !== 'Resolved').length} /><Stat label="Open Cases" value={reports.filter(report => report.status === 'Open').length} /></div><section className="case-section"><h2>Priority Cases</h2>{priority.length === 0 ? <p className="muted">No priority cases.</p> : <CaseTable reports={priority} />}</section><section className="case-section dashboard-recent"><h2>Recent Reports</h2>{recent.length === 0 ? <p className="muted">No reports created yet.</p> : <CaseTable reports={recent} />}</section></>}</section> }
function CaseTable({ reports }) { return <div className="table-wrap"><table><thead><tr><th>CASE</th><th>THREAT LEVEL</th><th>STATUS</th></tr></thead><tbody>{reports.map(report => <tr key={report._id}><td><b>{report.title}</b><small>{report.caseNumber}</small></td><td><span className={'badge ' + report.threatLevel.toLowerCase()}>{report.threatLevel}</span></td><td>{report.status}</td></tr>)}</tbody></table></div> }
